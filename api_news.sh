...
word_search="$1"
if [ -z "${word_search}" ];then
  echo "Usage: Requires a search term"
  echo "Using default search term : Trump"
  word_search="Trump"
else
  echo "Searching for term : ${word_search}"
fi
...
cat "${write_file}" |jq ".articles[] | select(.title | contains(\"${word_search}\")) | { title, url }"
